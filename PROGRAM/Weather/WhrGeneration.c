...
//		oldWind = rand(30);
	...
	float Amp1 = (5.0 + WIND2AMPLITUDE*(seaWindSpeed*Amp1rand + RAIN2AMPLITUDE*effectiveRain));
	...
	float foamrand = 3.0*(frnd()-0.5)*FOAMRANDOM;
	WeathersNH.Sea2.FoamV = Amp1*(0.65 + foamrand);
	...
	if (curTime <=5)
	{
		fblend = 1;
		transparency = 0;
		// WeathersNH.Sea2.FoamEnable = false;
		WeathersNH.Sea2.FoamV = Amp1*(0.69 + foamrand);
		WeathersNH.Sea2.FoamK = 0.1 - 0.05;
		WeathersNH.Sea2.FoamUV = scale1*0.5;
	}
